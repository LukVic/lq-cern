...
2.4011863773394473
    ...
    graph_all = ax_all.graph(x, lq_all, labelfmt="EP", options="", linecolor=root.kRed + 2)
    graph_max = ax_all.graph(x, lq_max, labelfmt="EP", options="", linecolor=root.kBlue)
    ...
    ax_all.plot(func_max, linecolor=root.kBlue,linestyle=root.kDashed, expand=False,
     label="LQ coresp. sep. mass. train ", labelfmt="L")

    ax_all.set_xlabel("Testing mass [GeV]")
    ax_all.set_ylabel("Significance [#sigma]")

    aplt.atlas_label(text="Internal", loc="upper left")
    ax_all.text(0.2, 0.81,"All masses VS One coresp.", align=10)
